...
- [tools/brightness_demo.py](tools/brightness_demo.py) / [tools/run_brightness_demo.sh](tools/run_brightness_demo.sh): visuelle Testsequenz fuer Helligkeit und Fading (laeuft per `mpremote run`, nichts wird geflasht).
...
5. Helligkeit und Fading am Panel pruefen (ca. 60 s, danach startet `main.py` automatisch neu):

```bash
./tools/run_brightness_demo.sh
```

Das Skript [tools/brightness_demo.py](tools/brightness_demo.py) spielt acht Tests ab (Treppe, langsamer
Fade, Atmen, sanfter und harter Textwechsel, Ruhetest gedimmt) und schreibt zu jedem auf die Konsole,
was auf dem Panel zu sehen sein soll. Wirken die unteren Stufen zu dunkel oder zu hell, `GAMMA` im
Skript aendern und den passenden Wert nach `BRIGHTNESS_GAMMA` in `settings.py` uebernehmen.
